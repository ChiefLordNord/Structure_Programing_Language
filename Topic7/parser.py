...
def parse_factor(tokens):
    ...
    if token["tag"] == "function":
        ast, tokens = parse_function_literal(tokens)
        return ast, tokens
...
def test_parse_expression_list():
    ...
    expression_list = "(" [ expression { "," expression } ] ")"
    """
    print("testing parse_expression_list...")
    # Test empty list
    tokens = tokenize("()")
...
def test_parse_identifier_list():
    ...
    identifier_list = "(" [ identifier { "," identifier } ] ")"
    """
    print("testing parse_identifier_list...")
    # Test empty list
    tokens = tokenize("()")
...
def parse_function_literal(tokens):
    ...
    statements, tokens = parse_statement_block(tokens)
    ast = {
        "tag":"function",
...
def test_parse_function_literal():
    ...
    function_literal = "function" identifier_list statement_block 
    """
    print("testing test_parse_function_literal...")
    ast, tokens = parse_function_literal(tokenize("function (x) {}"))
    assert ast == {'tag': 'function', 'parameters': {'tag': 'identifier_list', 'identifiers': [{'tag': 'identifier', 'value': 'x'}]}, 'statements': {'tag': 'block', 'statements': []}}
    assert tokens[0]["tag"] == None
